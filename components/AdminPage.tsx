...
const AdminPage = () => {
  ...
  const AdminUserRow = ({ user: u }: { user: User }) => (
    <tr className="border-b border-gray-700 hover:bg-brand-gray/50">
      ...
      <td className="p-3">
        <span
          className={`px-2 py-1 text-xs rounded-full ${u.role === "admin" ? "bg-green-800 text-green-200" : "bg-gray-600 text-gray-300"}`}
        >
          {u.role}
        </span>
      </td>
  ...
  const AdminContent = () => {
    ...
    if (error)
      return (
        <div className="text-red-400 p-8 text-center bg-red-900/20 rounded-lg">
          {error}
        </div>
      );
    ...
    switch (activeTab) {
      ...
      default:
        return <p className="text-gray-400">Select a section to manage.</p>;
    }
  };
        ...
        <nav className="flex flex-col space-y-2">
          {["users", "orders", "jobs", "coupons"].map((tab) => (
            <button
              ...
              className={`text-left p-3 rounded-lg capitalize transition-colors ${activeTab === tab ? "bg-brand-primary text-brand-dark font-bold" : "hover:bg-brand-gray/50 text-gray-300"}`}
            >
              {tab}
            </button>
          ))}
