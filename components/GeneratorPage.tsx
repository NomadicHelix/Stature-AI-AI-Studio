          ...
          >
            {stepIdx < currentStepIndex ? (
              <>
                ...
                <span className="relative flex h-8 w-8 items-center justify-center bg-brand-primary rounded-full">
                  <CheckCircleIcon
              ...
              <>
                ...
                >
                  <span
                    className="h-2.5 w-2.5 bg-brand-primary rounded-full"
                    aria-hidden="true"
                  />
                </span>
              </>
            ...
            <span className="absolute top-10 -left-2 w-16 text-center text-sm font-medium text-gray-400">
              {step.name}
            </span>
          </li>
        ))}
    ...
    <div className="w-full max-w-4xl mx-auto p-8 bg-brand-gray/50 rounded-2xl shadow-2xl border border-brand-primary/20">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-400 mt-2">
          Upload 5-10 recent photos. For best results, use a variety of angles
          and expressions.
        </p>
      </div>
      <div className="border-2 border-dashed border-gray-600 rounded-xl p-8 text-center bg-brand-dark/50 hover:border-brand-primary transition-colors">
        ...
        <label
          htmlFor="file-upload"
          className={`cursor-pointer flex flex-col items-center ${files.length >= 10 ? "opacity-50 cursor-not-allowed" : ""}`}
        >
          ...
          <p className="text-xs text-gray-500 mt-1">
            PNG or JPG | {files.length}/10 photos selected
          </p>
        </label>
      </div>
      {files.length > 0 && (
        <div className="mt-6">
          ...
          <button
            ...
            className="w-full mt-8 py-3 px-6 bg-brand-primary text-brand-dark text-lg font-bold rounded-lg flex items-center justify-center gap-2 hover:bg-brand-secondary transition-all transform hover:scale-105 disabled:bg-gray-600 disabled:cursor-not-allowed disabled:scale-100"
          >
            {files.length < 5
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-8 bg-brand-gray/50 rounded-2xl shadow-2xl border border-brand-primary/20">
      ...
      <p className="text-gray-400 text-center mb-8">
        Your{" "}
        <span
          className={`font-bold ${userPackage === "PRO" ? "text-brand-primary" : "text-brand-secondary"}`}
        >
          {userPackage || "Starter"}
        </span>{" "}
        plan allows up to{" "}
        <span className="font-bold text-white">{maxStyles}</span> style
        {maxStyles > 1 ? "s" : ""} and{" "}
        <span className="font-bold text-white">{maxImages}</span> headshots.
      </p>

      <div className="mb-8 p-6 bg-brand-dark/50 rounded-xl border border-brand-primary/30">
        <h3 className="text-xl font-semibold text-white mb-3 flex items-center gap-2">
          <SparklesIcon className="w-6 h-6 text-brand-primary" /> AI Style
          Assistant
        </h3>
        <p className="text-gray-400 mb-4">
          Describe your profession or desired vibe, and we'll suggest the
          perfect style.
        </p>
        <div className="flex gap-2">
          <input
            ...
            placeholder="e.g., 'Software Engineer' or 'Friendly and confident'"
            className="flex-grow bg-gray-700 text-white placeholder-gray-500 rounded-lg px-4 py-2 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-primary"
          />
          <button
            onClick={handleSuggestion}
            disabled={isLoading || !profession}
            className="px-6 py-2 bg-brand-primary text-brand-dark font-bold rounded-lg hover:bg-brand-secondary disabled:bg-gray-600 transition-colors flex items-center justify-center w-28"
          >
            {isLoading ? (
              <Spinner
                size="5"
                className="border-brand-dark border-t-transparent"
              />
            ) : (
              "Suggest"
            )}
      ...
      <p className="text-gray-400 text-center mb-6">
        ({selectedStyles.length}/{maxStyles} selected)
      </p>
        ...
        {STYLES.map((style) => {
          ...
          return (
            <div
              ...
              className={`bg-brand-dark rounded-xl overflow-hidden border-2 transition-all transform hover:-translate-y-2 cursor-pointer ${isSelected ? "border-brand-primary shadow-lg shadow-brand-primary/20" : suggestion?.id === style.id ? "border-brand-secondary" : "border-gray-700 hover:border-brand-primary/50"}`}
            >
              <div className="relative">
                ...
                {isSelected && (
                  <div className="absolute inset-0 bg-brand-primary/30 flex items-center justify-center">
                    <CheckCircleIcon className="w-16 h-16 text-white/80" />
                  </div>
                )}
              </div>
              <div className="p-4">
      ...
      {selectedStyles.length > 0 && (
        <div className="mt-8 pt-8 border-t border-brand-primary/20">
          ...
          <p className="text-gray-400 text-center mb-6">
            Select the total number of headshots and other preferences.
          </p>
          <div className="flex items-center justify-center gap-6 max-w-lg mx-auto">
            ...
            />
            <span className="text-2xl font-bold text-white bg-brand-dark/50 px-4 py-1 rounded-lg w-24 text-center">
              {imageCount}
            </span>
          </div>
      ...
      <button
        onClick={() =>
          onStylesSelected(selectedStyles, imageCount, removePiercings)
        }
        disabled={selectedStyles.length === 0}
        className="w-full max-w-md mx-auto mt-10 py-3 px-6 bg-brand-primary text-brand-dark text-lg font-bold rounded-lg flex items-center justify-center gap-2 hover:bg-brand-secondary transition-all transform hover:scale-105 disabled:bg-gray-600 disabled:cursor-not-allowed disabled:scale-100"
      >
...
const GenerationErrorView = ({
  ...
  <div className="text-center p-8 bg-red-900/30 rounded-2xl shadow-2xl border border-red-500/50 flex flex-col items-center justify-center min-h-[400px]">
    ...
    <div className="mt-8 flex flex-col sm:flex-row gap-4">
      <button
        onClick={onRetry}
        className="py-3 px-8 bg-brand-primary text-brand-dark font-bold rounded-lg hover:bg-brand-secondary transition-colors"
      >
        Try Again
      </button>
      <button
        onClick={onStartOver}
        className="py-3 px-8 bg-brand-gray text-white font-bold rounded-lg hover:bg-gray-600 transition-colors"
      >
        Start Over
      </button>
    </div>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-400 mt-2">
          Favorite and download your new professional look.
        </p>
      </div>

      {Object.entries(groupedImages).map(([styleName, images]) => (
        <section key={styleName} className="mb-16">
          <h3 className="text-3xl font-bold text-white mb-6 pb-2 border-b-2 border-brand-primary/30">
            {styleName} Style
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {images.map((image) => (
              ...
              >
                <img
                  src={image.src}
                  alt={`Generated Headshot in ${styleName} style`}
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
                  ...
                  >
                    <HeartIcon
                      className={`w-6 h-6 ${image.isFavorite ? "text-red-500" : "text-white"}`}
                      filled={image.isFavorite}
                    />
                  </button>
                  <a
                    href={image.src}
                    download={`stature-ai-headshot-${image.id}.png`}
                    className="p-3 bg-white/20 rounded-full hover:bg-white/30 backdrop-blur-sm transition-colors"
                    aria-label="Download"
                  >
      ...
      <div className="mt-12 text-center flex flex-col sm:flex-row justify-center gap-4">
        ...
        <button
          onClick={onReset}
          className="py-3 px-8 bg-brand-gray text-white text-lg font-bold rounded-lg hover:bg-gray-600 transition-colors"
        >
          Start Over
        </button>
      </div>
...
const GeneratorPage = ({
  ...
  const generationMessages = [
    ...
    "Calibrating the AI model...",
    "Crafting your professional look...",
  ...
  const handleStylesSelected = async (
    styles: HeadshotStyle[],
    count: number,
    piercings: boolean,
  ) => {
    ...
    const countsPerStyle = styles.map(
      (_, index) => imagesPerStyle + (index < remainder ? 1 : 0),
    );
    ...
    try {
      for (const [index, style] of styles.entries()) {
        if (styles.length > 1) {
          setLoadingMessage(
            `(${index + 1}/${styles.length}) Generating ${countsPerStyle[index]} images for '${style.name}' style...`,
          );
        }
