...
async function promoteUserToAdmin() {
  if (USER_ID_TO_PROMOTE === "poop" || !USER_ID_TO_PROMOTE) {
    console.error("ERROR: Please paste the User ID of the user you want to promote into the 'USER_ID_TO_PROMOTE' variable in this script.");
    return;
  }
  ...
  try {
    ...
    await admin.auth().setCustomUserClaims(USER_ID_TO_PROMOTE, { admin: true, role: 'admin' });
    ...
    if (error.code === 'auth/user-not-found') {
        console.error("This user does not exist in Firebase Authentication. Please check the UID.");
    }
  }
}
